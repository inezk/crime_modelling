...
class logistic_regression(Predictor):
    def __init__(self, data, penalty = 'l2',reg_constant = 1.0, feature_cols = [],
    ...
    def predict(self, moving_window, start_index, end_index, lag_window):
        ...
        row = 0
        for i in range((start_index- lag_window) * n_space, (end_index - lag_window)* n_space, n_space):
            training_chunk = frame[i - (moving_window * n_space):i,:]
            self.model.fit(training_chunk[:,4:].astype(int), training_chunk[: ,0].astype(int))
            predictions = self.model.predict(frame[i: i + n_space, 4:])
            results[row: row + n_space, 0] = "LOGISTIC REGRESSION"
