...
class SpatialDataSet(object):
    def __init__(self, id_col, time_col, coords_cols, feature_cols, type_col, date_format = "%Y/%d/%m",
        ...
        (self.counts, self.ID) = self.make_feature_frame(id_col, time_col, type_col)
        self.coords = np.array(coords_cols) #nd_array of nrow x 2, 
    ...
    def make_feature_frame(self, id_col, time_col, type_col): #output is 3-d numpy array
        id_dict = dict() 
        id_row = 0
        ...
        for i in range(0, len(unique_id)):
            ...
            id_dict[i] = unique_id[i] #frame index : spatial ID
            id_row += 1
        return (counts_frame, id_dict)
